...
class C_pernikahan extends CI_Controller
{
    ...
    {
        $data = array(
            'title'  => 'List pernikahan',
            'menu'   => 'pernikahan',
            'script' => 'pernikahan',
            'konten' => 'admin/pernikahan/list'
        );
    ...
    {
        error_reporting(0);
        $list = $this->pernikahan->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->pernikahan->count_all(),
            "recordsFiltered" => $this->pernikahan->count_filtered(),
            "data" => $data,
        );
    ...
    {
        $data = $this->req->all();
        if ($this->pernikahan->insert($data) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function update()
    {
        ...
        if ($this->pernikahan->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->pernikahan->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    {
        if ($action == 'on') {
            if ($this->pernikahan->update(['status' => '1'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->pernikahan->update(['status' => '0'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pernikahan->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pernikahan->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pernikahan->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
