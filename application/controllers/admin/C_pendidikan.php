...
class C_pendidikan extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/M_pendidikan', 'pendidikan');
        $this->pendidikan->upk = $this->session->userdata('UPK');
    }
    ...
    {
        $data = $this->req->all();
        if ($this->pendidikan->insert($data) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function update()
    {
        ...
        if ($this->pendidikan->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->pendidikan->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    {
        if ($action == 'on') {
            if ($this->pendidikan->update(['status' => '1'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->pendidikan->update(['status' => '0'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pendidikan->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pendidikan->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->pendidikan->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
