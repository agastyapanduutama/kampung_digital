...
class C_rt extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('admin/M_rt', 'rt');
        $this->rt->upk = $this->session->userdata('UPK');
    }
    ...
    function data()
    {
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->rt->count_all(),
            "recordsFiltered" => $this->rt->count_filtered(),
            "data" => $data,
        );
    ...
    function update()
    {
        ...
        if ($this->rt->update($data, $this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    function delete($id)
    {
        if ($this->rt->delete($this->req->id($id)) == true) {
            $msg = array(
                'status' => 'ok',
    ...
    {
        if ($action == 'on') {
            if ($this->rt->update(['status' => '1'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
            ...
            if ($this->rt->update(['status' => '0'], $this->req->id($id)) == true) {
                $msg = array(
                    'status' => 'ok',
    ...
    function allOff()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->rt->update(['status' => 0], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allOn()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->rt->update(['status' => 1], $this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
    ...
    function allDelete()
    {
        ...
        foreach ($idNa as $id) {
            if ($id != "") {
                if ($this->rt->delete($this->req->id($id)) == true) $jmlNa++;
            }
        }
        if ($jmlNa > 0) {
            echo json_encode([
