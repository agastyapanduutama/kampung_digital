$(document).ready(function () {
	table = $('#list_pendidikan').DataTable({
		"processing": true,
		"serverSide": true,
		"order": [],

		"ajax": {
			"url": baseUrl + 'admin/pendidikan/data',
			"type": "POST",
			"complete": function () {
				cekPilihanNa()
			},
...
$('#list_pendidikan').on('click', '#edit', function () {
	let id = $(this).data('id');
	$.ajax({
		url: baseUrl + 'admin/pendidikan/get/' + id,
		type: "GET",
		success: function (result) {
			response = JSON.parse(result)
			$("#idData").val(response.id)
			$("#pendidikan1").val(response.nama_pendidikan)
...
$('#list_pendidikan').on('click', '#delete', function () {
	let id = $(this).data('id');
	confirmSweet("Data akan terhapus secara permanen !")
		.then(result => {
			if (result) {
				$.ajax({
					url: baseUrl + 'admin/pendidikan/delete/' + id,
					type: "GET",
					success: function (result) {
						response = JSON.parse(result)
...
$("#formAddpendidikan").submit(function (e) {
	e.preventDefault();
	$.ajax({
		url: baseUrl + "admin/pendidikan/insert",
		type: "post",
...
$("#formEditpendidikan").submit(function (e) {
	e.preventDefault();
	$.ajax({
		url: baseUrl + "admin/pendidikan/update",
		type: "post",
		data: new FormData(this),
		processData: false,
		contentType: false,
...
$('#list_pendidikan').on('click', '#on', function () {
    let id = $(this).data('id');
    confirmSweet("Data akan dinon-aktifkan  !")
        .then(result => {
            if (result) {
                $.ajax({
                    url: baseUrl + 'admin/pendidikan/set/' + id + "/off",
                    type: "GET",
                    success: function (result) {
                        response = JSON.parse(result)
                        if (response.status == 'ok') {
...
$('#list_pendidikan').on('click', '#off', function () {
    let id = $(this).data('id');
    confirmSweet("Data akan diaktifkan  !")
        .then(result => {
            if (result) {
                $.ajax({
                    url: baseUrl + 'admin/pendidikan/set/' + id + "/on",
                    type: "GET",
                    success: function (result) {
                        response = JSON.parse(result)
                        if (response.status == 'ok') {
...
$("#btnAllOff").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dinon-aktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pendidikan/allOff`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllOn").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan diaktifkan  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pendidikan/allOn`,
					data: {
						id: dipilih
					},
					type: "post",
...
$("#btnAllHapus").on("click", function () {
	...
	if (dipilih == '') {
		...
		confirmSweet(`${jml.length - 1} Akun akan dihapus  !`).then(respon => {
			if (respon) {
				$.ajax({
					url: `${baseUrl}/admin/pendidikan/allDelete`,
					data: {
						id: dipilih
					},
					type: "post",
